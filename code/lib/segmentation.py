...
#==============================================================================
    ...
    def polygon(self,slopes,ref,  limit  ):
       ...
       for k in range(limit):
           for i in range(line):
               for j in range(col):
                   alpha = (slopes[0][k]*i+slopes[1][k]*j+slopes[2][k])*ref[0,k]
                   alpha_positif = (alpha >= 0)
                   if alpha_positif.all():
                       # this condition should be if for all k alpha_positif is true
                       if (k == (limit-1)):
                           res[i,j]=True
                       else:
                           break
...
#==============================================================================
    ...
    def forearmLeft(self,A,B):
        ...
        p1=B[pos2D[5,1],pos2D[5,0]]#(pos2D(6,2),pos2D(6,1));
        p2=B[pos2D[6,1],pos2D[6,0]]#(pos2D(7,2),pos2D(7,1));
        ...
        #cross product 
        t = np.cross(np.insert(vect_elbow, -1,0),np.insert(vect65, -1,0))
        t1 = np.cross(np.insert(vect_215,-1,0),np.insert(-vect65,-1,0))
        if t1[2]>0:
